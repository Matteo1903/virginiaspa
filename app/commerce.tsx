...
export default function CommerceExperience({ language, mode = "overview" }: { language: Language; mode?: "overview" | "treatments" | "gift" }) {
  ...
  const addGift = () => {
    if (gift.delivery === "date" && !giftDate) return;
    const delivery = gift.delivery === "date" ? giftDate : "now";
    setCart((items) => [...items, { id: `gift-${Date.now()}`, title: "Virginia SPA Gift Card", detail: gift.to ? `${l.recipient}: ${gift.to}` : l.amount, price: giftAmount, quantity: 1, gift: { ...gift, delivery } }]);
    setStage("cart");
    setCartOpen(true);
  };
    ...
    <section id={mode === "gift" ? "gift-atelier" : "shop"} className={mode === "gift" ? "commerce-section gift-commerce-page" : "commerce-section"}>
      ...
      <div className="gift-builder">
        ...
        <div className="gift-form">
          ...
          {gift.delivery === "date" && <label>{l.date}<input type="date" min={new Date().toISOString().slice(0, 10)} required value={giftDate} onChange={(event) => setGiftDate(event.target.value)} /></label>}
          <button className="button button-primary" type="button" onClick={addGift} disabled={gift.delivery === "date" && !giftDate}>{l.addGift}<span>→</span></button>
        </div>
      </div>
      }
