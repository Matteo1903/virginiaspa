...
export function SiteFooter({ language = "it" }: { language?: Language }) {
  ...
  return <footer>
    ...
    <div className="footer-links"><div><span>{text.explore}</span><Link href="/#shop">{text.treatments}</Link><Link href="/chi-siamo">{language === "it" ? "Chi siamo" : language === "en" ? "About us" : language === "es" ? "Quiénes somos" : language === "fr" ? "Qui sommes-nous" : "Über uns"}</Link><Link href="/#metodo">{text.method}</Link><Link href="/gift-card">Gift Card</Link></div><div><span>{text.contacts}</span><a href={spaPhoneHref}>{spaPhoneDisplay}</a><a href={`mailto:${spaEmail}`}>{spaEmail}</a><Link href="/">Instagram ↗</Link></div><div><span>{text.hours}</span><p>{text.days}<br />09:00 — 20:00</p></div></div>
