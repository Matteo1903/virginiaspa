...
const copy: Record<Language, { back: string; experience: string; duration: string; price: string; provisional: string; buy: string; checkout: string; name: string; email: string; phone: string; secure: string; cancel: string; error: string }> = {
  it: { back: "Torna alle esperienze", experience: "Esperienza Virginia SPA", duration: "Durata", price: "Prezzo", provisional: "Valori provvisori", buy: "Acquista il voucher", checkout: "Continua con Stripe", name: "Nome e cognome", email: "Email", phone: "Telefono", secure: "Pagamento sicuro sulla pagina protetta di Stripe.", cancel: "Annulla", error: "Non è stato possibile avviare il pagamento." },
  en: { back: "Back to experiences", experience: "Virginia SPA experience", duration: "Duration", price: "Price", provisional: "Provisional values", buy: "Buy the voucher", checkout: "Continue with Stripe", name: "Full name", email: "Email", phone: "Phone", secure: "Secure payment on Stripe’s protected page.", cancel: "Cancel", error: "We could not start the payment." },
  es: { back: "Volver a experiencias", experience: "Experiencia Virginia SPA", duration: "Duración", price: "Precio", provisional: "Valores provisionales", buy: "Comprar el bono", checkout: "Continuar con Stripe", name: "Nombre completo", email: "Email", phone: "Teléfono", secure: "Pago seguro en la página protegida de Stripe.", cancel: "Cancelar", error: "No se ha podido iniciar el pago." },
  fr: { back: "Retour aux expériences", experience: "Expérience Virginia SPA", duration: "Durée", price: "Prix", provisional: "Valeurs provisoires", buy: "Acheter le bon", checkout: "Continuer avec Stripe", name: "Nom complet", email: "E-mail", phone: "Téléphone", secure: "Paiement sécurisé sur la page protégée de Stripe.", cancel: "Annuler", error: "Impossible de démarrer le paiement." },
  de: { back: "Zurück zu den Erlebnissen", experience: "Virginia-SPA-Erlebnis", duration: "Dauer", price: "Preis", provisional: "Vorläufige Werte", buy: "Gutschein kaufen", checkout: "Weiter mit Stripe", name: "Vollständiger Name", email: "E-Mail", phone: "Telefon", secure: "Sichere Zahlung auf der geschützten Stripe-Seite.", cancel: "Abbrechen", error: "Die Zahlung konnte nicht gestartet werden." },
};
...
export default function RitualExperiencePage({ experience }: { experience: RitualExperience }) {
  const [language, setLanguage] = useSiteLanguage(); const [cart, setCart] = useState<StoredCartItem[]>([]); const [cartOpen, setCartOpen] = useState(false); const [stage, setStage] = useState<"cart" | "checkout">("cart"); const [loading, setLoading] = useState(false); const [error, setError] = useState("");
  ...
  const updateQuantity = (id: string, change: -1 | 1) => { const next = cart.map((item) => item.id === id ? { ...item, quantity: Math.max(1, Math.min(10, item.quantity + change)) } : item); setCart(next); writeStoredCart(next); };
  const checkout = async (event: FormEvent<HTMLFormElement>) => { event.preventDefault(); setLoading(true); setError(""); const form = new FormData(event.currentTarget); try { const response = await fetch("/api/checkout", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ name: form.get("name"), email: form.get("email"), phone: form.get("phone"), language, items: cart, acceptedTerms: form.get("acceptedTerms") === "on" }) }); const data = await response.json() as { url?: string; error?: string }; if (!response.ok || !data.url) throw new Error(data.error || text.error); location.assign(data.url); } catch (cause) { setError(cause instanceof Error ? cause.message : text.error); setLoading(false); } };
  return <main className="ritual-page-shell"><SiteHeader language={language} onLanguageChange={setLanguage} showBooking={false} extraAction={<button className="cart-trigger ritual-header-cart" type="button" onClick={() => { setStage("cart"); setCartOpen(true); }} aria-label={`${cartText.cart}: ${cart.reduce((sum, item) => sum + item.quantity, 0)}`}><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M3 4h2l2 11h10l2-7H7M9 20h.01M17 20h.01" /></svg><span>{cartText.cart}</span><i>{cart.reduce((sum, item) => sum + item.quantity, 0)}</i></button>} />
    ...
    {cartOpen && <div className="commerce-backdrop" onMouseDown={() => setCartOpen(false)}><aside className="cart-drawer" role="dialog" aria-modal="true" aria-labelledby="ritual-cart-title" onMouseDown={(event) => event.stopPropagation()}><button className="commerce-close" type="button" onClick={() => setCartOpen(false)} aria-label={cartText.close}>×</button>{stage === "cart" ? <><p className="section-index">Virginia SPA</p><h2 id="ritual-cart-title">{cartText.cart}</h2><PurchaseNotice language={language} />{cart.length === 0 ? <div className="cart-empty"><p>{cartText.empty}</p></div> : <><div className="cart-items">{cart.map((item) => <article key={item.id}><div><h3>{item.title}</h3><p>{item.detail}</p><div className="cart-item-actions"><div className="quantity-stepper" aria-label={`${translate("Quantità", language)}: ${item.title}`}><button type="button" onClick={() => updateQuantity(item.id, -1)} disabled={item.quantity === 1} aria-label={`${translate("Riduci quantità", language)}: ${item.title}`}>−</button><output aria-live="polite">{item.quantity}</output><button type="button" onClick={() => updateQuantity(item.id, 1)} disabled={item.quantity === 10} aria-label={`${translate("Aumenta quantità", language)}: ${item.title}`}>+</button></div><button className="cart-remove" type="button" onClick={() => removeFromCart(item.id)}>{cartText.remove}</button></div></div><strong>{euro.format(item.price * item.quantity)}</strong></article>)}</div><div className="cart-total"><span>{cartText.total}</span><strong>{euro.format(total)}</strong></div><button className="button button-primary commerce-primary" type="button" onClick={() => setStage("checkout")}>{cartText.proceed}<span>→</span></button></>}</> : <form className="checkout-form" onSubmit={checkout}><p className="section-index">Stripe · Virginia SPA</p><h2 id="ritual-cart-title">{cartText.proceed}</h2><PurchaseNotice language={language} /><fieldset><legend>{cartText.details}</legend><label>{text.name}<input name="name" required autoComplete="name" maxLength={120} /></label><label>{text.email}<input name="email" required type="email" autoComplete="email" maxLength={254} /></label><label>{text.phone}<input name="phone" required type="tel" autoComplete="tel" maxLength={40} /></label></fieldset><label className="privacy-check"><input name="acceptedTerms" type="checkbox" required /><LegalConsent language={language} /></label><p className="demo-payment"><span>✓</span>{text.secure}</p>{error && <p className="checkout-error" role="alert">{error}</p>}<div className="checkout-summary"><span>{cartText.total}</span><strong>{euro.format(total)}</strong></div><button className="button button-primary commerce-primary" disabled={loading}>{loading ? "Stripe…" : text.checkout}<span>→</span></button></form>}</aside></div>}
